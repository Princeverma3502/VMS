...
const Header = ({ onOpenCommand }) => {
  const { logoUrl, marqueeText } = useBranding();
        ...
        {logoUrl && (
          <div className="flex items-center gap-2">
            <img 
              ...
              className="h-12 w-12 min-w-[3rem] min-h-[3rem] bg-white border border-slate-200 rounded-full object-contain p-1 shadow" 
            />
          </div>
        )}
      ...
      <div className="bg-gradient-to-r from-blue-700 to-indigo-700 border-t border-blue-600/30 py-2">
        <div className="max-w-5xl mx-auto px-4 text-sm text-white overflow-hidden">
          <div className="whitespace-nowrap animate-marquee font-semibold tracking-wide">
            {marqueeText || '🔥 No important messages at the moment'}
          </div>
        </div>
      </div>
    </header>
